...
\State $\mathbf{x}_{masked} \gets \mathbf{x}$ \Comment{Copy input sequence}
\State $\mathbf{y} \gets [-100, -100, \ldots, -100]$ \Comment{Initialize labels (ignore non-masked)}
\State $N_{mask} \gets \lfloor n \times p_{mask} \rfloor$ \Comment{Number of positions to mask}
\State $\mathcal{I} \gets \textsc{RandomSample}(\{1, 2, \ldots, n\}, N_{mask})$ \Comment{Select positions to mask}

\For{$i \in \mathcal{I}$} \Comment{Process each selected position}
    \State $y_i \gets x_i$ \Comment{Store original token for loss computation}
    \State $r \gets \textsc{Random}()$ \Comment{Draw random number $r \in [0, 1)$}
    
    \If{$r < 0.8$} \Comment{80\% of time: replace with [MASK]}
        \State $x_{masked,i} \gets \text{MASK\_ID}$
    \ElsIf{$r < 0.9$} \Comment{10\% of time: replace with random token}
        \State $x_{masked,i} \gets \textsc{RandomInt}(1, V)$
    \Else \Comment{10\% of time: keep original token}
...
\State $t \gets 0$ \Comment{Current training step}

\Function{GetMaskingProbability}{}
    \If{$t < T_{warmup}$}
        \State $\text{progress} \gets t / T_{warmup}$ \Comment{Linear warmup schedule}
        \State \Return $p_{init} + (p_{final} - p_{init}) \times \text{progress}$
    \Else
        \State \Return $p_{final}$ \Comment{Full masking probability after warmup}
    \EndIf
\EndFunction

\Function{Step}{}
    \State $t \gets t + 1$ \Comment{Increment training step counter}
\EndFunction
