...
\begin{comment}
Feedback: This is a good introduction. To make it more engaging, you could frame it as a narrative about a core problem in NLP. For example: "The [UNK] token is more than just a placeholder; it is the embodiment of a fundamental tension in natural language processing: the conflict between the finite vocabulary of a machine learning model and the infinite, ever-evolving nature of human language."

STATUS: addressed - reframed opening to emphasize fundamental tension between finite model vocabulary and infinite language
\end{comment}
...
\begin{comment}
Feedback: This is a great transition. Before listing the methods, it would be powerful to state the core insight that unites them. For example: "The key insight behind the subword revolution was that even rare and unknown words are often composed of common, meaningful sub-units. By breaking words down into these smaller pieces, a model could maintain a fixed vocabulary while still representing a virtually unlimited set of words."

STATUS: addressed - added core insight about subword decomposition enabling unlimited word representation with fixed vocabulary
\end{comment}
...
\end{principle}
\begin{comment}
Feedback: This is a good list. To make it more actionable: For "Monitor Coverage," you could suggest: "Before fine-tuning on a new domain, run a vocabulary coverage analysis. If the [UNK] rate is above a certain threshold (e.g., 1%), consider augmenting the tokenizer's vocabulary with new domain-specific tokens." For "Handle Gracefully," you could add: "For generative tasks, implement a copy mechanism or a constrained decoding strategy to prevent the model from outputting literal '[UNK]' strings, which are unintelligible to users."
\end{comment}
