...
\foreach \i in {0,...,8} {
    \foreach \j in {0,...,8} {
        \pgfmathsetmacro{\opacity}{0.1 + 0.8*random()}
        \node[rectangle, fill=bertblue, opacity=\opacity, minimum size=0.3cm] at (2.2+\i*0.2, 2.2+\j*0.2) {};
    }
}
...
\draw[arrow, very thick, bertblue] (1.5, 4.5) -- (1.5, 4);
\node[label, bertblue] at (1.5, 4.2) {Local};

\draw[arrow, very thick, sepviolet] (4, 4.5) -- (4, 4);
\node[label, sepviolet] at (4, 4.2) {Bridge};

\draw[arrow, very thick, gptgreen] (6.5, 4.5) -- (6.5, 4);
