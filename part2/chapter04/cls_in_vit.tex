...
\centering
\begin{tikzpicture}[
    patch/.style={rectangle, minimum width=0.8cm, minimum height=0.8cm, draw=gray},
    clstoken/.style={circle, minimum width=1cm, fill=clsorange!20, draw=clsorange, thick},
...
\foreach \x in {0,1,2,3} {
    \foreach \y in {0,1,2,3} {
        \node[patch, fill=bertblue!10] at (\x*0.9, \y*0.9 + 6) {};
    }
}

% CLS tokens for different layers
\node[clstoken] (cls1) at (5, 8) {\cls{}};
\node[clstoken] (cls2) at (9, 8) {\cls{}};
\node[clstoken] (cls3) at (13, 8) {\cls{}};

% Layer labels
\node[layer] at (1.35, 5.3) {Image Patches};
\node[layer] at (5, 5.3) {Layer 2};
\node[layer] at (9, 5.3) {Layer 6};
\node[layer] at (13, 5.3) {Layer 12};

% Attention patterns - Early layer (broad)
\foreach \x in {0,1,2,3} {
    \foreach \y in {0,1,2,3} {
        \draw[attention, bertblue!50] (\x*0.9 + 0.4, \y*0.9 + 6.4) to[bend left=20] (cls1);
    }
}

% Attention patterns - Middle layer (focused)
\draw[attention, gptgreen, very thick] (0.4, 7.3) to[bend left=30] (cls2);
\draw[attention, gptgreen, very thick] (1.3, 7.3) to[bend left=25] (cls2);
\draw[attention, gptgreen, very thick] (0.4, 8.2) to[bend left=35] (cls2);
\draw[attention, gptgreen!50] (2.2, 6.4) to[bend left=15] (cls2);
\draw[attention, gptgreen!30] (3.1, 6.4) to[bend left=10] (cls2);

% Attention patterns - Late layer (discriminative)
\draw[attention, maskred, very thick] (1.3, 8.2) to[bend left=20] (cls3);
\draw[attention, maskred, thick] (0.4, 7.3) to[bend left=25] (cls3);
\draw[attention, maskred!50] (2.2, 7.3) to[bend left=15] (cls3);

% Labels
\node[font=\tiny, align=center] at (5, 4.5) {Broad\\uniform\\attention};
\node[font=\tiny, align=center] at (9, 4.5) {Focused\\semantic\\attention};
\node[font=\tiny, align=center] at (13, 4.5) {Discriminative\\feature\\attention};
