...
\Function{GetMaskingRatio}{$t$} \Comment{$t$ is current training step}
    \If{$t \geq T$} \Comment{Training complete}
        \State \Return $r_{final}$
    \EndIf
    
    \State $\text{progress} \gets t / T$ \Comment{Training progress as fraction}
    \State $\text{cosine\_factor} \gets \frac{1}{2}(1 + \cos(\pi \times \text{progress}))$ \Comment{Cosine annealing}
...
\textbf{Training Loop:}
\For{$t = 1, 2, \ldots, T$}
    \State $r_{current} \gets \textsc{GetMaskingRatio}(t)$ \Comment{Get current masking ratio}
